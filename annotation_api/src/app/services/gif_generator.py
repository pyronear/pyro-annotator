...
class SequenceGifGenerator:
    ...
    async def generate_gifs_for_annotation(self, annotation_id: int) -> dict:
        ...
        updated_annotation_data = SequenceAnnotationData(
            sequences_bbox=updated_sequences_bbox
        )
    ...
    async def _generate_gifs_for_bbox(
        self, bbox_data: SequenceBBox, detections: List[Detection]
    ) -> dict:
        """
        ...
        for detection in relevant_detections:
            try:
                ...
                logger.warning(
                    f"Failed to load image for detection {detection.id}: {str(e)}"
                )
                continue
    ...
    def _crop_image(
        self, image: np.ndarray, xyxyn: List[float], padding: float = 0.1
    ) -> np.ndarray:
    ...
    def _create_gif_bytes(
        self, frames: List[np.ndarray], duration: float = 0.5
    ) -> bytes:
        """
    ...
    async def _update_annotation_gifs(
        self, annotation_id: int, annotation_data: SequenceAnnotationData
    ) -> None:
