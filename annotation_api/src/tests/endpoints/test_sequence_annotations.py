...
@pytest.mark.asyncio
async def test_list_sequence_annotations_filter_by_smoke_types(
    authenticated_client: AsyncClient,
):
    """Test filtering sequence annotations by smoke types."""
    # Create test sequences
    test_sequences = [
        {
            "source_api": "pyronear_french",
            "alert_api_id": "9201",
            "camera_name": "Smoke Anno Test Camera 1",
            "camera_id": "921",
            "organisation_name": "Test Org",
            "organisation_id": "1",
            "lat": "43.5",
        ...
        },
        {
            "source_api": "pyronear_french",
            "alert_api_id": "9202",
            "camera_name": "Smoke Anno Test Camera 2",
            "camera_id": "922",
            "organisation_name": "Test Org",
            "organisation_id": "1",
            "lat": "43.0",
        ...
        },
        {
            "source_api": "pyronear_french",
            "alert_api_id": "9203",
            "camera_name": "Smoke Anno Test Camera 3",
            "camera_id": "923",
            "organisation_name": "Test Org",
            "organisation_id": "1",
            "lat": "43.5",
    ...
    # Create sequence annotations with different smoke types
    ...
    # Test 1: Filter by single smoke type - should match first annotation
    response = await authenticated_client.get(
        "/annotations/sequences/?smoke_types=wildfire"
    )
    assert response.status_code == 200
    data = response.json()
    filtered_annotations = data["items"]
    ...
    # Test 2: Filter by multiple smoke types - should match first two annotations
    response = await authenticated_client.get(
        "/annotations/sequences/?smoke_types=wildfire&smoke_types=other"
    )
    assert response.status_code == 200
    data = response.json()
    filtered_annotations = data["items"]
    ...
    # Test 3: Filter by smoke type that matches multiple annotations - industrial also in first annotation
    response = await authenticated_client.get(
        "/annotations/sequences/?smoke_types=industrial"
    )
    assert response.status_code == 200
    data = response.json()
    filtered_annotations = data["items"]

    # Should match first annotation (which has both wildfire and industrial)
    matching_ids = [
        ann["id"] for ann in filtered_annotations if ann["id"] in annotation_ids
    ]
    assert annotation_ids[0] in matching_ids
    ...
    response = await authenticated_client.get(
        "/annotations/sequences/?smoke_types=nonexistent"
    )
    assert response.status_code == 422
    error_data = response.json()
    assert "detail" in error_data
...
@pytest.mark.asyncio
async def test_list_sequence_annotations_smoke_types_validation(
    authenticated_client: AsyncClient,
):
    ...
    response = await authenticated_client.get(
        "/annotations/sequences/?smoke_types=invalid_type"
    )
    assert response.status_code == 422

    # Test with mix of valid and invalid types
    response = await authenticated_client.get(
        "/annotations/sequences/?smoke_types=wildfire&smoke_types=invalid_type"
    )
    assert response.status_code == 422
