...
async def test_auto_create_detection_annotations_correct_structure(
    async_client: AsyncClient, sequence_session: AsyncSession, detection_session, mock_img: bytes
):
    ...
    response = await async_client.post("/annotations/sequences/", json=sequence_annotation_payload)
    assert response.status_code == 201, f"Failed to create sequence annotation: {response.text}"
    ...
    assert detection_1_annotation is not None, "Detection annotation should be auto-created for detection_id 1"
    assert detection_2_annotation is not None, "Detection annotation should be auto-created for detection_id 2"
    ...
    assert "annotation" in detection_1_annotation, "Detection annotation should have 'annotation' field"
    assert isinstance(detection_1_annotation["annotation"], dict), "Annotation field should be a dict"
    assert "annotation" in detection_1_annotation["annotation"], "Annotation field should have 'annotation' key"
    assert isinstance(detection_1_annotation["annotation"]["annotation"], list), "Annotation.annotation should be a list"
    assert detection_1_annotation["annotation"]["annotation"] == [], "Annotation.annotation should be an empty list initially"

    # Same verification for second detection
    assert "annotation" in detection_2_annotation, "Detection annotation should have 'annotation' field"
    assert isinstance(detection_2_annotation["annotation"], dict), "Annotation field should be a dict"
    assert "annotation" in detection_2_annotation["annotation"], "Annotation field should have 'annotation' key"
    assert isinstance(detection_2_annotation["annotation"]["annotation"], list), "Annotation.annotation should be a list"
    assert detection_2_annotation["annotation"]["annotation"] == [], "Annotation.annotation should be an empty list initially"

    # Step 5: Verify processing_stage is set correctly (should be 'bbox_annotation' since has_smoke=True)
    assert detection_1_annotation["processing_stage"] == "bbox_annotation", "Processing stage should be bbox_annotation for smoke detection"
    assert detection_2_annotation["processing_stage"] == "bbox_annotation", "Processing stage should be bbox_annotation for smoke detection"
...
async def test_auto_create_detection_annotations_update_scenario(
    ...
    detection_1_exists_initially = any(ann["detection_id"] == 1 for ann in initial_annotations)
    assert not detection_1_exists_initially, "Detection annotation should not exist initially"
    ...
    assert detection_1_annotation is not None, "Detection annotation should be auto-created after update to 'annotated'"

    # Step 5: Verify the correct annotation structure
    assert detection_1_annotation["annotation"]["annotation"] == [], "Auto-created annotation should have correct empty structure"
    assert detection_1_annotation["processing_stage"] == "bbox_annotation", "Processing stage should be bbox_annotation for smoke detection"
...
async def test_auto_create_detection_annotations_processing_stages(
            ...
            "sequences_bbox": [
                {
                    ...
                    "gif_key_main": None,
                    "gif_key_crop": None,
                    "false_positive_types": [models.FalsePositiveType.ANTENNA.value],  # Has false positives
                    "bboxes": [{"detection_id": 1, "xyxyn": [0.1, 0.1, 0.2, 0.2]}],
                }
            ]
        },
    ...
    response1 = await async_client.post("/annotations/sequences/", json=payload_visual_check)
    assert response1.status_code == 201
    
    # Check the created detection annotation has visual_check stage
    detection_annotations_response = await async_client.get("/annotations/detections/")
    detection_annotations = detection_annotations_response.json()["items"]
    detection_1_annotation = next(ann for ann in detection_annotations if ann["detection_id"] == 1)
    assert detection_1_annotation["processing_stage"] == "visual_check", "Should be visual_check for false positives only"
            ...
            "sequences_bbox": [
                {
                    ...
                    "gif_key_crop": None,
                    "false_positive_types": [],
                    "bboxes": [{"detection_id": detection2_id, "xyxyn": [0.1, 0.1, 0.2, 0.2]}],
                }
            ]
        },
        "processing_stage": models.SequenceAnnotationProcessingStage.ANNOTATED.value,
    ...
    response2 = await async_client.post("/annotations/sequences/", json=payload_bbox_annotation)
    assert response2.status_code == 201

    # Check the created detection annotation has bbox_annotation stage
    detection_annotations_response = await async_client.get("/annotations/detections/")
    detection_annotations = detection_annotations_response.json()["items"]
    detection_2_annotation = next((ann for ann in detection_annotations if ann["detection_id"] == detection2_id), None)
    assert detection_2_annotation is not None, "Detection annotation should be created"
    assert detection_2_annotation["processing_stage"] == "bbox_annotation", "Should be bbox_annotation for smoke detection"
            ...
            "sequences_bbox": [
                {
                    ...
                    "gif_key_crop": None,
                    "false_positive_types": [],
                    "bboxes": [{"detection_id": detection3_id, "xyxyn": [0.1, 0.1, 0.2, 0.2]}],
                }
            ]
        },
        "processing_stage": models.SequenceAnnotationProcessingStage.ANNOTATED.value,
    ...
    response3 = await async_client.post("/annotations/sequences/", json=payload_missed_smoke)
    assert response3.status_code == 201

    # Check the created detection annotation has bbox_annotation stage
    detection_annotations_response = await async_client.get("/annotations/detections/")
    detection_annotations = detection_annotations_response.json()["items"]
    detection_3_annotation = next((ann for ann in detection_annotations if ann["detection_id"] == detection3_id), None)
    assert detection_3_annotation is not None, "Detection annotation should be created"
    assert detection_3_annotation["processing_stage"] == "bbox_annotation", "Should be bbox_annotation for missed smoke"
...
async def test_detection_annotation_update_workflow(
    ...
    seq_response = await async_client.post("/annotations/sequences/", json=sequence_annotation_payload)
    assert seq_response.status_code == 201

    # Step 2: Get the auto-created detection annotation
    detection_annotations_response = await async_client.get("/annotations/detections/")
    detection_annotations = detection_annotations_response.json()["items"]
    detection_annotation = next(ann for ann in detection_annotations if ann["detection_id"] == 1)
    ...
    assert update_response.status_code == 200, f"Failed to update detection annotation: {update_response.text}"
    ...
    get_response = await async_client.get(f"/annotations/detections/{detection_annotation_id}")
...
async def test_detection_annotation_validation_requirements(
    ...
    valid_response = await async_client.post("/annotations/detections/", data=valid_payload)
    assert valid_response.status_code == 201, "Valid annotation structure should be accepted"
    ...
    invalid_payload_empty = {
        "detection_id": str(validation_detection_id),
        "annotation": json.dumps({}),  # Invalid: empty object instead of {"annotation": [...]}
        "processing_stage": "visual_check",
    }

    # This should fail with validation error due to invalid structure
    invalid_response_empty = await async_client.post("/annotations/detections/", data=invalid_payload_empty)
    assert invalid_response_empty.status_code == 422  # Validation error for invalid structure
    ...
    assert invalid_update_response.status_code == 422, "Invalid annotation structure should be rejected"
    ...
    invalid_bbox_response = await async_client.patch(
        f"/annotations/detections/{detection_annotation_id}",
        json=invalid_bbox_payload
    )
    assert invalid_bbox_response.status_code == 422, "Invalid bbox coordinates should be rejected"
    ...
    assert invalid_smoke_response.status_code == 422, "Invalid smoke type should be rejected"
...
async def test_auto_create_avoids_duplicate_detection_annotations(
    ...
    manual_response = await async_client.post("/annotations/detections/", data=manual_payload)
            ...
            "sequences_bbox": [
                {
                    ...
                    "false_positive_types": [],
                    "bboxes": [
                        {"detection_id": 1, "xyxyn": [0.1, 0.1, 0.2, 0.2]},  # Already has annotation
                        {"detection_id": 2, "xyxyn": [0.3, 0.3, 0.4, 0.4]}   # No existing annotation
                    ],
                }
            ]
        },
    ...
    get_existing_response = await async_client.get(f"/annotations/detections/{existing_annotation_id}")
    ...
    assert existing_annotation["annotation"]["annotation"][0]["smoke_type"] == "wildfire"
    ...
    detection_2_annotations = [ann for ann in all_annotations if ann["detection_id"] == 2]
    assert len(detection_2_annotations) == 1, "Should create exactly one annotation for detection_id=2"
    
    detection_2_annotation = detection_2_annotations[0]
    assert detection_2_annotation["annotation"]["annotation"] == [], "New auto-created annotation should have empty structure"
...
async def test_no_auto_create_when_not_annotated_stage(
    ...
    detection_annotations_response = await async_client.get("/annotations/detections/")
    detection_annotations = detection_annotations_response.json()["items"]
    detection_1_annotations = [ann for ann in detection_annotations if ann["detection_id"] == 1]
    assert len(detection_1_annotations) == 0, "No detection annotations should be created when not in 'annotated' stage"
    ...
    seq_response_2 = await async_client.post("/annotations/sequences/", json=sequence_annotation_payload_2)
    assert seq_response_2.status_code == 201

    # Verify still no detection annotations were created
    detection_annotations_response = await async_client.get("/annotations/detections/")
    detection_annotations = detection_annotations_response.json()["items"]
    detection_2_annotations = [ann for ann in detection_annotations if ann["detection_id"] == 2]
    assert len(detection_2_annotations) == 0, "No detection annotations should be created for 'ready_to_annotate' stage"