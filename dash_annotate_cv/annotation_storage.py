...
class AnnotationStorage(DataClassDictMixin):
    ...
    def __post_init__(self):
        if StorageType.JSON in self.storage_types:
            assert (
                self.json_file is not None
            ), "json_file must be set if storage_type is JSON"
        if StorageType.COCO in self.storage_types:
            assert (
                self.coco_file is not None
            ), "coco_file must be set if storage_type is COCO"
...
class AnnotationWriter:
    ...
    def write(self, annotations: Any):
        ...
        write_every = (
            self.storage.storage_frequency
            == AnnotationStorage.StorageFrequency.EVERY_OPERATION
        )
        write_every_n = (
            self.storage.storage_frequency
            == AnnotationStorage.StorageFrequency.EVERY_N_OPERATIONS
            and self._ctr_write % self.storage.storage_frequency_every_n == 0
        )
        ...
        if StorageType.JSON in self.storage.storage_types:
            assert (
                self.storage.json_file is not None
            ), "json_file must be set if storage_type is JSON"
        ...
        if StorageType.COCO in self.storage.storage_types:
            assert (
                self.storage.coco_file is not None
            ), "coco_file must be set if storage_type is COCO"
...
def load_image_anns_from_storage(
    storage: AnnotationStorage,
) -> Optional[ImageAnnotations]:
    if len(storage.storage_types) == 0:
        return None
    for storage_type in storage.storage_types:
        anns = load_image_anns_if_exist(
            storage_type, json_file=storage.json_file, coco_file=storage.coco_file
        )
        if anns is not None:
            return anns
    return None
