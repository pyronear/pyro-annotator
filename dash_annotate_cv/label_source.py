...
class LabelSource(DataClassDictMixin):
    ...
    def __post_init__(self):
        if self.source_type == LabelSource.Type.DEFAULT:
            ...
            ), "labels must be set if source_type is DEFAULT"
        elif self.source_type == LabelSource.Type.JSON_FILE:
            assert (
                self.json_file is not None
            ), "json_file must be set if source_type is JSON_FILE"
        elif self.source_type == LabelSource.Type.TXT_FILE:
            assert (
                self.txt_file is not None
            ), "txt_file must be set if source_type is TXT_FILE"
        else:
    ...
    def get_labels(self) -> List[str]:
        ...
        if self.source_type == LabelSource.Type.DEFAULT:
            ...
            with open(self.json_file) as f:
                ...
                assert all(
                    [type(item) == str for item in data]
            ...
            with open(self.txt_file) as f:
                ...
                assert all(
                    [type(item) == str for item in data]
                ), "TXT file must contain a list of strings"
                return data
        else:
            raise NotImplementedError
