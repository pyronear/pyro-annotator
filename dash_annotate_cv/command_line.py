...
class Conf(DataClassDictMixin):
    ...
    options: dacv.AnnotateImageOptions = field(
        default_factory=dacv.AnnotateImageOptions
    )
...
def cli():
    ...
    parser.add_argument(
        "conf",
        type=str,
        help="Path to the configuration file YAML file. See docs for details on the format.",
    )
    ...
    if conf.mode == Conf.Mode.IMAGE_LABELS:
        ...
        app.layout = dbc.Container([html.H1("Annotate Images"), aio])
        ...
        app.layout = dbc.Container([html.H1("Annotate Bounding Boxes"), aio])
    else:
