...
def write_to_coco(anns: ImageAnnotations, fname_output_json: str):
    assert (
        os.path.splitext(fname_output_json)[1] == ".json"
    ), "fname_output_json must be a json file"
    ...
    for ann in anns.image_to_entry.values():
        ...
        if ann.image_width is None or ann.image_height is None:
            logger.warning(
                f"Skipping writing image with no specified width or height to COCO format: {ann}"
            )
            continue
        ...
        for bbox in ann.bboxs or []:
            ...
            if bbox.class_name is None:
                logger.warning(
                    f"Skipping writing bbox with no class name to COCO format: {bbox}"
                )
                continue
            ...
            if bbox.area_normalized(ann.image_width, ann.image_height) <= 0:
                logger.warning(
                    f"Skipping writing bbox with area <= 0 to COCO format: {bbox}"
                )
                continue
            ...
            cat_id = [
                cat["id"]
                for cat in coco_dct["categories"]
                if cat["name"] == bbox.class_name
            ][0]
...
def load_from_coco_if_exist(fname_json: str) -> Optional[ImageAnnotations]:
    ...
    for ann in coco_dct["annotations"]:
        ...
        xywh_unnormalized = unnormalize_xywh(
            xywh_normalized, img_dct["width"], img_dct["height"]
        )
        xyxy_unnormalized = xywh_to_xyxy(xywh_unnormalized)

        # Add bounding box
        bbox = ImageAnnotations.Annotation.Bbox(
            xyxy=xyxy_unnormalized, class_name=class_name
        )
