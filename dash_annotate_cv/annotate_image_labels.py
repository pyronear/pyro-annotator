...
class AnnotateImageLabelsAIO(html.Div):
    ...
    def _define_callbacks(self):
        ...
        def change_label(dropdown_value):
            try:
                ...
                if is_initial:
                    return (
                        self._create_image_layout(),
                        self._create_existing_label_alert_layout(),
                    )
                    ...
                    if self.selection_mode == SelectionMode.SINGLE:
                        ...
                        raise NotImplementedError(
                            f"Unknown selection mode: {self.selection_mode}"
                        )
    ...
    def _create_layout(self):
        ...
        instructions_txt = (
            ...
            or "Label this image from the following options:"
        )
        ...
        return dbc.Row(
            [
                ...
                dbc.Col([html.Div(id=self.ids.image(self.aio_id))], md=6),
                dbc.Col([instructions, dropdown], md=6, class_name="align-self-center"),
            ]
        )
    ...
    def _create_existing_label_alert_layout(self) -> Optional[dbc.Alert]:
        ...
        if existing_label is not None:
            if type(existing_label) == str and existing_label in self.controller.labels:
                return dbc.Alert(
                    f"Existing annotation: {existing_label}", color="primary"
                )
            elif type(existing_label) == list and all(
                [l in self.controller.labels for l in existing_label]
            ):
                return dbc.Alert(
                    f"Existing annotation: {existing_label}", color="primary"
                )
                ...
                return dbc.Alert(
                    f"Existing unknown annotation: {existing_label}", color="danger"
                )
        else:
            return None
