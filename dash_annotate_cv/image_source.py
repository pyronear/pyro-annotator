...
class ImageSource(DataClassDictMixin):
    ...
    images: Optional[List[Tuple[str, Image.Image]]] = field(
        default=None,
...
class ImageIterator:
    ...
    def __init__(self, image_source: ImageSource):
        ...
        if image_source.source_type == ImageSource.Type.FOLDER:
            ...
            assert (
                image_source.folder_name is not None
            ), "folder_name must be set if source_type is FOLDER"
            self._file_names = glob.glob(
                os.path.join(image_source.folder_name, image_source.folder_pattern)
            )
            self.no_images = len(self._file_names)
        elif image_source.source_type == ImageSource.Type.LIST_OF_FILES:
            assert (
                image_source.list_of_files is not None
            ), "list_of_files must be set if source_type is LIST_OF_FILES"
            self._file_names = image_source.list_of_files
            self.no_images = len(self._file_names)
        else:
            assert (
                image_source.images is not None
            ), "images must be set if source_type is DEFAULT"
    ...
    def _image_at_idx(self, idx: int) -> Tuple[int, str, Image.Image]:
        ...
        if self.image_source.source_type == ImageSource.Type.DEFAULT:
            assert (
                self.image_source.images is not None
            ), "images must be set if source_type is DEFAULT"
            ret = self.image_source.images[idx]
            return idx, ret[0], ret[1]
        else:
            assert (
                self._file_names is not None
            ), "file_names must be set if source_type is not DEFAULT"
