...
export interface DrawingCanvasState {
  ...
  selectRectangleAtPoint: (point: Point, rectangles: DrawnRectangle[], imageBounds: ImageBounds) => void;
  updateSelectedRectangleSmokeType: (rectangles: DrawnRectangle[], smokeType: SmokeType) => DrawnRectangle[];
...
export const useDrawingCanvas = (config: Partial<DrawingCanvasConfig> = {}): DrawingCanvasState => {
  ...
  const finishDrawing = useCallback((imageBounds: ImageBounds, smokeType: SmokeType): DrawnRectangle | null => {
  ...
  const deleteSelectedRectangle = useCallback((rectangles: DrawnRectangle[]): DrawnRectangle[] => {
    if (!selectedRectangleId) return rectangles;
  ...
  const pushUndoState = useCallback((rectangles: DrawnRectangle[]) => {
