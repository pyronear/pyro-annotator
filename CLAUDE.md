...
Each alert also carries `temporal_model_score` (plus `temporal_model_version` / `temporal_api_version`) for score-based mining: rank objects by their alert's score and filter on `record_kind` to surface hard negatives. The score is alert-level because the platform's temporal model scores an alert, not an object — the verdict rides the primary lane and object-split siblings hold NULL. `null` means **never scored** (alerts imported before 2026-08-10, fail-opens, risk-gated sequences), never "scored low", so drop nulls when ranking rather than coalescing them to `0.0`; `0.0` itself is a real verdict.
